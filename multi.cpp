...
/* structure used for passing arguments to the function */
typedef struct {
	npc *npcs;
...
} process_continent_args;

/* This function offloads the work of processing a continent onto multiple threads */
static void *process_continent(void *v_args) {
	process_continent_args args = *(process_continent_args*)v_args;
	for (uint64_t it = 0; it < args.num_iterations; ++it) {
		/* move all of the npcs in this continent. */
		move_npcs(args.npcs, args.npcs_per_continent, args.npc_speed);
		/* move back in bounds if need be */
		move_in_bounds(args.npcs, args.npcs_per_continent);
		/* give them new directions if need be */
		give_new_dir(args.npcs, args.npcs_per_continent, it);
		/* check if they interacted */
		interact_with_all(args.npcs, args.npcs_per_continent, args.range_to_interact);
	}
}

#ifndef TEST_SINGLE_

uint64_t start_multi(uint64_t seed, uint16_t num_continents, uint64_t npcs_per_continent, float range_to_interact, uint64_t num_iterations, float npc_speed, uint8_t disp) {
	...
	/* The amount of threads is based on the amount of continents */
	pthread_t *tids = (pthread_t*) malloc(num_continents * sizeof(pthread_t));
	process_continent_args *args_list = (process_continent_args*) malloc(num_continents * sizeof(process_continent_args));

	for (uint16_t cont = 0; cont < num_continents; ++cont) {
		args_list[cont].npcs = npcs[cont];
		args_list[cont].npcs_per_continent = npcs_per_continent;
		args_list[cont].range_to_interact = range_to_interact;
		args_list[cont].npc_speed = npc_speed;
		args_list[cont].num_iterations = num_iterations;
		pthread_create(&tids[cont], NULL, process_continent, (void*)&args_list[cont]);
	}

	/* join threads */
	for (uint16_t cont = 0; cont < num_continents; ++cont) {
		pthread_join(tids[cont], NULL);
	}
